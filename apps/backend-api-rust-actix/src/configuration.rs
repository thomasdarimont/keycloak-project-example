...
impl Configuration {
    pub fn from_environment_with_defaults() -> Self {
        Self {
            ...
            cert_location: env::var("CERT_LOCATION")
                .unwrap_or("../../config/stage/dev/tls/acme.test+1.pem".into()),
            key_location: env::var("KEY_LOCATION")
                .unwrap_or("../../config/stage/dev/tls/acme.test+1-key.pem".into()),
            oidc_issuer: env::var("OIDC_ISSUER")
                .unwrap_or("https://id.acme.test:8443/auth/realms/acme-internal".into()),
            allowed_cors_origin: env::var("ALLOWED_CORS_ORIGIN")
                .unwrap_or("https://apps.acme.test:4443".into()),
        }
    }
}
