...
public class DefaultAuthenticationFlow implements AuthenticationFlow {
    ...
    void fillListsOfExecutions(Stream<AuthenticationExecutionModel> executionsToProcess, List<AuthenticationExecutionModel> requiredList, List<AuthenticationExecutionModel> alternativeList) {
        ...
        // Patch:Begin applyUserPrecendenceToAlternativesIfNecessary
        applyUserPrecendenceToAlternativesIfNecessary(alternativeList);
        // Patch:End
    ...
    private void applyUserPrecendenceToAlternativesIfNecessary(List<AuthenticationExecutionModel> alternativeList) {

        if (alternativeList.isEmpty()) {
            return;
        }

        var user = this.processor.getAuthenticationSession() == null ? null : this.processor.getAuthenticationSession().getAuthenticatedUser();
        if (user == null) {
            return;
        }

        // acme-auth-preference=authenticator-id1,authenticator-id2,...
        // we need at least 2 authenticators to compute a partial ordering
        var authenticatorPreferenceString = user.getFirstAttribute("acme-auth-preference");
        if (authenticatorPreferenceString == null || authenticatorPreferenceString.trim().isEmpty()) {
            return;
        }
        var authPreferenceList = List.of(authenticatorPreferenceString.split(","));
        if (authPreferenceList.isEmpty() || authPreferenceList.size() == 1) {
            return;
        }
